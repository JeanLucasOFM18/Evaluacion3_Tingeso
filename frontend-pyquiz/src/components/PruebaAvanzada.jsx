...
const PruebaAvanzada = () =>{
    ...
    useEffect(() => {
        const obtenerPreguntas = async () => {
          try {
            const response = await axios.get('http://localhost:8080/repositorio/obtenerPorNivel/Avanzado');
    ...
    const renderizarResultado = () => {
      ...
      let descripcion = "";
      if (puntajeTotal === 7) {
          descripcion = "Lograste superar el nivel Avanzado, ya eres un maestro en Python";
      } else if (puntajeTotal >= 4) {
          descripcion = "Estás a punto de superar este desafío, vuelve a intentarlo";
      } else {
          descripcion = "Te falta repasar los conceptos de Python para superar este desafío";
      }
